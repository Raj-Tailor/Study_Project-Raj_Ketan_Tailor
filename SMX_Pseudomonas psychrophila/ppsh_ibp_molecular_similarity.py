...
# Path to Ibuprofen.mol
Ibuprofen_file_path = os.path.join(current_dir, 'Ibuprofen.mol')

# Read Ibuprofen molecule
Ibuprofen_molecule = read_molecule(Ibuprofen_file_path)
...
for mol_filename in os.listdir(current_dir):
    if mol_filename.endswith('.mol') and mol_filename != 'Ibuprofen.mol':
        molecule_file_path = os.path.join(current_dir, mol_filename)
        try:
            mol = read_molecule(molecule_file_path)
            similarity_score = calculate_similarity(Ibuprofen_molecule, mol)
...
output_file_path = os.path.join(current_dir, 'ppsh_ibp_similarity_results.xlsx')
...
output_sorted_file_path = os.path.join(current_dir, 'ppsh_ibp_similarity_results_sorted.xlsx')
