...
# Path to Carbamazepine.mol
Carbamazepine_file_path = os.path.join(current_dir, 'Carbamazepine.mol')

# Read Carbamazepine molecule
Carbamazepine_molecule = read_molecule(Carbamazepine_file_path)
...
for mol_filename in os.listdir(current_dir):
    if mol_filename.endswith('.mol') and mol_filename != 'Carbamazepine.mol':
        molecule_file_path = os.path.join(current_dir, mol_filename)
        try:
            mol = read_molecule(molecule_file_path)
            similarity_score = calculate_similarity(Carbamazepine_molecule, mol)
...
output_file_path = os.path.join(current_dir, 'ppsh_cmp_similarity_results.xlsx')
...
output_sorted_file_path = os.path.join(current_dir, 'ppsh_cmp_similarity_results_sorted.xlsx')
