...
# Path to Atenolol.mol
Atenolol_file_path = os.path.join(current_dir, 'Atenolol.mol')

# Read Atenolol molecule
Atenolol_molecule = read_molecule(Atenolol_file_path)
...
for mol_filename in os.listdir(current_dir):
    if mol_filename.endswith('.mol') and mol_filename != 'Atenolol.mol':
        molecule_file_path = os.path.join(current_dir, mol_filename)
        try:
            mol = read_molecule(molecule_file_path)
            similarity_score = calculate_similarity(Atenolol_molecule, mol)
...
output_file_path = os.path.join(current_dir, 'smiz_ate_similarity_results.xlsx')
...
output_sorted_file_path = os.path.join(current_dir, 'smiz_ate_similarity_results_sorted.xlsx')
